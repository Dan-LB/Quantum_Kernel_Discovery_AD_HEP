...
def plot_ROC_kfold_mean(
    ...
    for i, id_name in enumerate(ids):  # for each latent space or train size
        ...
        print("PLOT.PY 174: correction division")
        corr = 0.0001

        auc_data_q = get_mean_and_error(np.array(auc_q))
        auc_data_c = get_mean_and_error(np.array(auc_c))

        #print(np.array(fpr_q))

        fpr_data_q = get_mean_and_error(1.0 / (np.array(fpr_q)+corr))
        fpr_data_c = get_mean_and_error(1.0 / (np.array(fpr_c)+corr))
    ...
    print("PLOT.PY 228: correction linspace")
    plt.semilogy(
        np.linspace(0, 1, num=int(1e4)),
        1.0 / np.linspace(0.0001, 1, num=int(1e4)),
        linewidth=1.5,
        linestyle="--",
        color="0.75",
    )

    if comparison_mode == CLASSICAL:
        label = "Classical Kernel"
    if comparison_mode == FIXED:
        label = "Fixed Kernel"

    legend1 = plt.legend(
        dummy_res_lines,
        [r"Learned Kernel", label],
        frameon=False,
        loc="upper right",
        handlelength=1.5,
        fontsize=16,
    ...
    legend2 = plt.legend(
        ...
        loc="lower left",
        frameon=True,
        title=r"AUC $\;\quad$Learned Kernel $\quad\;\;\;$ "+label,
        fontsize=15,
        title_fontsize=14,
        markerscale=0.5,
    )
...
def create_table_for_fixed_TPR(
    ...
    """
    # output to latex
    df_output = pd.DataFrame({"1/FPR": ["Quantum", "Fixed"]})
