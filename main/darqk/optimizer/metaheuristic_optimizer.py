...
class MetaheuristicOptimizer(BaseKernelOptimizer):
    ...
    def __init__(self, initial_kernel: Kernel, X: np.ndarray, y: np.ndarray, ke: KernelEvaluator, env = "TIGHT"):
        super().__init__(initial_kernel, X, y, ke)

        def cost(array):
            array = array.ravel().astype(int)
            the_array = np.concatenate([array, np.array([initial_kernel.type])]).astype(int)
        ...
        self.env = env
        self.space = self.get_opytimize_space(self.env).astype(int)
        self.optimizer = PSO() #QUA VIENE SELEZIONATO OPT
        #params = { #params for GA
        #    "p_selection": 0.75,
        #    "p_mutation": 0.25,
        #    "p_crossover": 0.5,
        #}
       ...
       # self.optimizer = GA(params=params)
    ...
    def optimize(self, n_iterations=1, verbose=False):
    ...
    def get_opytimize_space(self, env = "TIGHT"):
        ...
        if env == "TIGHT":
            allowed_generators = ['IX', 'IY', 'IZ', 'XY', 'XZ', 'YZ']
        if env == "WIDE":
            allowed_generators = self.initial_kernel.get_allowed_operations()

        n_variables = 5 * n_operations + n_qubits
        step = [1, 1, 1, 1, 1] * n_operations + [1] * n_qubits #0.33 è la bw minima
        lower_bound = [0, 0, 0, 0, 1] * n_operations + [0] * n_qubits
        upper_bound = [len(allowed_generators) - 1, n_qubits - 1, n_qubits - 2, n_features, 1] * n_operations + [3] * n_qubits
